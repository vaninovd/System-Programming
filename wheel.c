...
char **prune_word_list(char **words, int len, int *words_remaining) {
    ...
    *words_remaining = j;
    char **new_words = malloc(sizeof(char*) * (j+1));
    int q = 0;
    int w = 0;
    while(words[q] != NULL){
        if(strlen(words[q]) == len){
            new_words[w] = words[q];
            w++;
        }
        q++;
    }
    new_words[*words_remaining] = NULL; 
    return new_words;
}
...
char **get_word_list_of_length(char **words, int *len) {
    ...
    *remaining = 0;
    char **word_list = prune_word_list(words, *len, remaining);
    while(*remaining == 0){
         printf("There are no words of that length.\n");
         fgets(buf, 1000, stdin);
         *len = strtol(buf, NULL, 10);
         word_list = prune_word_list(words, *len, remaining);
    }
    free(remaining);
    return word_list;
}
